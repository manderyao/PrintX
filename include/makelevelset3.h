...
static float point_segment_distance(const Vec3f &x0, const Vec3f &x1, const Vec3f &x2, Vec3f &pt)
{
	Vec3f dx(x2 - x1);
	double m2 = mag2(dx);
...
static float point_triangle_distance(const Vec3f &x0, const Vec3f &x1, const Vec3f &x2, const Vec3f &x3, Vec3f &pt)
{
	...
	if (w23 >= 0 && w31 >= 0 && w12 >= 0){ // if we're inside the triangle
		...
		{
			d1 = point_segment_distance(x0, x1, x2, pt1);
			d2 = point_segment_distance(x0, x1, x3, pt2);
			if (d1 < d2)
			{
				pt = pt1;
				return d1;
		...
		{
			d1 = point_segment_distance(x0, x1, x2, pt1);
			d2 = point_segment_distance(x0, x2, x3, pt2);
			if (d1 < d2)
			{
				pt = pt1;
				return d1;
		...
		{
			d1 = point_segment_distance(x0, x1, x3, pt1);
			d2 = point_segment_distance(x0, x2, x3, pt2);
			if (d1 < d2)
			{
				pt = pt1;
...
static void check_neighbour(const std::vector<Vec3ui> &tri, const std::vector<Vec3f> &x,
	Array3f &phi, Array3i &closest_tri, Array3vec3f &closest_pt,
	const Vec3f &gx, int i0, int j0, int k0, int i1, int j1, int k1)
{
	if (closest_tri(i1, j1, k1) >= 0){
		unsigned int p, q, r; assign(tri[closest_tri(i1, j1, k1)], p, q, r);
		Vec3f pt;
		float d = point_triangle_distance(gx, x[p], x[q], x[r], pt);
		if (d < phi(i0, j0, k0)){
			phi(i0, j0, k0) = d;
			closest_tri(i0, j0, k0) = closest_tri(i1, j1, k1);
			closest_pt(i0, j0, k0) = pt;
		}
	}
}

static void sweep(const std::vector<Vec3ui> &tri, const std::vector<Vec3f> &x,
	Array3f &phi, Array3i &closest_tri, Array3vec3f &closest_pt, const Vec3f &origin, float dx,
	int di, int dj, int dk)
{
	...
	for (int k = k0; k != k1; k += dk) for (int j = j0; j != j1; j += dj) for (int i = i0; i != i1; i += di){
		...
		check_neighbour(tri, x, phi, closest_tri, closest_pt, gx, i, j, k, i - di, j, k);
		check_neighbour(tri, x, phi, closest_tri, closest_pt, gx, i, j, k, i, j - dj, k);
		check_neighbour(tri, x, phi, closest_tri, closest_pt, gx, i, j, k, i - di, j - dj, k);
		check_neighbour(tri, x, phi, closest_tri, closest_pt, gx, i, j, k, i, j, k - dk);
		check_neighbour(tri, x, phi, closest_tri, closest_pt, gx, i, j, k, i - di, j, k - dk);
		check_neighbour(tri, x, phi, closest_tri, closest_pt, gx, i, j, k, i, j - dj, k - dk);
		check_neighbour(tri, x, phi, closest_tri, closest_pt, gx, i, j, k, i - di, j - dj, k - dk);
	}
}
...
static void make_level_set3(const std::vector<Vec3ui> &tri, const std::vector<Vec3f> &x,
	const Vec3f &origin, float dx, int ni, int nj, int nk,
	Array3f &phi, Array3i &closest_tri, Array3vec3f &closest_pt, const int exact_band=1)
{
	phi.resize(ni, nj, nk);
	...
	intersection_count.resize(ni, nj, nk);
	intersection_count.assign(0);

	closest_pt.resize(ni, nj, nk);
	...
	for (unsigned int pass = 0; pass < 2; ++pass){
		sweep(tri, x, phi, closest_tri, closest_pt, origin, dx, +1, +1, +1);
		sweep(tri, x, phi, closest_tri, closest_pt, origin, dx, -1, -1, -1);
		sweep(tri, x, phi, closest_tri, closest_pt, origin, dx, +1, +1, -1);
		sweep(tri, x, phi, closest_tri, closest_pt, origin, dx, -1, -1, +1);
		sweep(tri, x, phi, closest_tri, closest_pt, origin, dx, +1, -1, +1);
